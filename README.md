...
- **RSSニュース取り込み**（`scripts/sync-news.mjs`）: Yahoo!ニュース・NHKなどのRSSから記事を取り込み、`/news`フィードに表示（後述）
...
なお現状、**異議申し立て側には保証金がありません**。嫌がらせの異議は精算を遅らせるだけでポイントを奪えないため当面は許容していますが、荒らしが発生した場合に最初に保証金を課すべき箇所です。

### RSSニュース取り込み

```bash
npm run sync-news   # 設定したRSSフィードから記事を取り込む（guid/linkで冪等）
```
