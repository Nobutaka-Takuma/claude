...
- [ ] 年齢に関する方針を決める（換金性がないため賭博には当たらない想定ですが、対象年齢は決めておくべきです）。規約の文面は`MINIMUM_AGE` / `PARENTAL_CONSENT_AGE`で変わります
...
- [ ] 規約・ポリシーの文面が事業の実態に合っているか確認する。雛形のままの箇所がないか、特に免責の範囲と個人情報の取扱い
- [ ] 広告・アクセス解析を導入したら、`/privacy`の「5. 利用者情報の外部送信について」に送信先の事業者名・送信される情報・利用目的を追記する（導入前は空欄で構いませんが、導入後に空欄のままにしないこと）
